...
class BrowserWorker:
    ...
    def setup_logging(self):
        if not logging.getLogger().handlers:
            logging.basicConfig(
                ...
                handlers=[
                    logging.FileHandler('youtube_engagement.log'),
    ...
    def post_comment(self, stream_url: str, comment: str) -> bool:
        """Post a single comment"""
        try:
            if not self.is_logged_in and not self.login():
                return False

            logging.info(f"Navigating to stream as {self.username}")
            self.page.goto(stream_url)
            self.page.wait_for_selector('video', timeout=60000)
            self.page.wait_for_load_state('networkidle', timeout=60000)
            time.sleep(random.uniform(3, 5))

            chat_frame = self.page.frame_locator('iframe#chatframe')
            if not chat_frame:
                expand_chat = self.page.get_by_role('button', name='Show chat').first
                if expand_chat:
                    expand_chat.click()
                    time.sleep(random.uniform(2, 3))
                    chat_frame = self.page.frame_locator('iframe#chatframe')

            chat_input = chat_frame.locator(
            ...
            for char in comment:
                chat_input.type(char, delay=random.randint(50, 200))
            time.sleep(random.uniform(1, 2))

            send_button = chat_frame.locator('#send-button > yt-button-renderer > yt-button-shape > button').first
    ...
    def run(self):
        ...
        try:
            while not self.stop_flag:
                ...
                stream_url, comment = self.comment_queue.get()
                if self.post_comment(stream_url, comment):
                    time.sleep(random.uniform(5, 10))  # Wait between comments
                else:
                    # If posting fails, try to re-login once
                    self.is_logged_in = False
                    if self.login():
                        self.post_comment(stream_url, comment)
...
class CommentManager:
    ...
    def distribute_comments(self, stream_url: str, comments: List[str]):
        ...
        for i, comment in enumerate(comments):
            ...
            worker.comment_queue.put((stream_url, comment))
            time.sleep(random.uniform(1, 3))  # Small delay between distributions
...
def main():
    ...
    stream_url = input("Enter the YouTube stream URL: ")
    if not stream_url:
    ...
    try:
        ...
        manager.distribute_comments(stream_url, comments)
